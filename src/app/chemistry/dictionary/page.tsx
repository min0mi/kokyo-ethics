...
function DictionaryContent() {
  ...
  const tags = useMemo(() => Array.from(new Set(scopedKeywords.flatMap((kw) => kw.tags || []))), [scopedKeywords]);
  const colorMap = useMemo(() => new Map(FIGURES.map((fig) => [fig.id, fig])), []);
  const filtered = useMemo(() => searchCatalog(query, tag, phase).filter((kw) => availableCategoryIds.has(kw.categoryId)), [query, tag, phase, availableCategoryIds]);
  ...
  const tagLabels = (values?: string[]) => !!values?.length && (
    <div className="mt-2 flex flex-wrap gap-1">
      {values.map((value) => <span key={value} className="rounded-full bg-gray-100 px-2 py-0.5 text-xs font-normal text-gray-500">#{value}</span>)}
    </div>
  );
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-3 px-2 py-3 text-xs text-gray-900 sm:px-4">
      ...
      <div className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
        ...
        <details>
          <summary className="cursor-pointer text-sm text-gray-600">タグで絞り込む（任意）{tag && `：${tag === '__untagged' ? 'タグなし' : tag}`}</summary>
          <div className="mt-3 flex flex-wrap gap-2">
            {[['', 'すべて'], ...tags.map((value) => [value, value]), ['__untagged', 'タグなし']].map(([value, label]) => (
              <button type="button" key={value} aria-pressed={tag === value} onClick={() => setTag(value)} className={buttonClass(tag === value)}>{label}</button>
            ))}
          </div>
        </details>
      </div>
      <p className="text-sm text-gray-500" role="status">{filtered.length}項目・{colors.length}色を表示{tag && '（タグで絞り込み中）'}</p>
      {filtered.length === 0 ? (
        <div className="rounded-lg border border-gray-200 bg-white p-8 text-center">
          <p>該当する物質がありません。</p>
          <button type="button" onClick={() => { setQuery(''); setTag(''); setPhase(''); }} className="mt-3 text-sm text-blue-700 underline">すべての絞り込みを解除</button>
        </div>
      ) : view === 'substance' ? (
          ...
          <ul className="divide-y divide-gray-100">
            {filtered.map((kw) => (
              <li key={kw.id} className="grid grid-cols-[minmax(0,1fr)_6rem] items-start gap-3 px-4 py-3 even:bg-gray-50/50 sm:grid-cols-[minmax(0,1fr)_9rem]">
                <div className="min-w-0 break-words"><strong className="text-base"><ChemicalText text={kw.name} /></strong>{kw.condition && <p className="mt-1 text-xs leading-relaxed text-gray-600"><ChemicalText text={kw.condition} /></p>}{tagLabels(kw.tags)}</div>
            ...
            <section key={color.id} className="overflow-hidden rounded border border-gray-300 bg-white shadow-xs">
              ...
              <table className="w-full border-collapse text-left">
                ...
                <tbody className="divide-y divide-gray-200">
                  {color.substances.map((kw, index) => <tr key={kw.id} className={index % 2 ? 'bg-gray-50/40' : 'bg-white'}><td className="border-r border-gray-200 px-3 py-2.5 text-base font-bold"><ChemicalText text={kw.name} /></td><td className="px-3 py-2.5 text-[11px] leading-relaxed text-gray-700">{kw.condition ? <ChemicalText text={kw.condition} /> : '—'}{tagLabels(kw.tags)}</td></tr>)}
                </tbody>
              </table>
            </section>
          ))}
