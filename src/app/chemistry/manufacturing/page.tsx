...
function makeQ(m:Mode,i:number):Q { const dryingOnly=m==='gas_to_drying'; const base=GAS_PROCESSES.filter(x=>!dryingOnly||DRYING_FORMULAS.includes(x.formula)); const p=base[i%base.length], g=GAS_PROPERTIES.find(x=>x.formula===p.formula)!;
 ...
 const availableAgents=GAS_DRYING_AGENTS.filter(a=>GAS_PROPERTIES.some(x=>DRYING_FORMULAS.includes(x.formula)&&x.drying.includes(a))),a=availableAgents[i%availableAgents.length],cs=GAS_PROPERTIES.filter(x=>DRYING_FORMULAS.includes(x.formula)&&x.drying.includes(a)),t=cs[i%cs.length];return{id:`dry-gas-${a}-${t.formula}`,prompt:`${a}で乾燥できる気体はどれか。`,options:[gasText(t),...GAS_PROPERTIES.filter(x=>!x.drying.includes(a)).slice(i%5,i%5+3).map(gasText)],answer:gasText(t)};
}
