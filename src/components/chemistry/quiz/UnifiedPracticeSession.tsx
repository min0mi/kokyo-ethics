...
export const UnifiedPracticeSession: React.FC<UnifiedPracticeProps> = ({
  ...
  const startSessionWithConfig = useCallback((cfg: QuizSessionConfig) => {
    // Filter the full bank before limiting the question count, otherwise a weak
    // question outside the random sample is incorrectly reported as absent.
    let pool = QuestionGenerator.generateCustomSession(cfg.onlyWeak ? { ...cfg, questionCount: 999 } : cfg);
    ...
    if (cfg.onlyWeak) {
      ...
      if (cfg.questionCount < 999) pool = pool.slice(0, cfg.questionCount);
    }
      ...
      {!isCompleted ? (
        currentQ ? (
          <div className="space-y-2">
            {/* 上部ヘッダー */}
            <div className="flex items-center justify-between rounded-xs border border-gray-400 bg-white px-3.5 py-2 text-xs shadow-xs">
